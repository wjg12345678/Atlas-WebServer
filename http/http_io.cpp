...
bool http_conn::ensure_read_capacity(int required_size)
{
    if (required_size <= 0)
    {
    ...
    if (old_size > 0 && m_read_ring.buffer != NULL)
    {
...
bool http_conn::ensure_write_capacity(int required_size)
{
    if (required_size <= 0)
    {
    ...
    vector<char> new_ring_buf(new_capacity, '\0');
    if (old_size > 0 && m_write_ring.buffer != NULL)
    {
