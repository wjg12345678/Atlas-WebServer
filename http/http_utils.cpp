...
bool http_conn::decode_base64(const string &input, string &output) const
{
    string cleaned;
...
string http_conn::header_param_value(const string &header_line, const string &key) const
{
    string pattern = key + "=";
...
string http_conn::escape_sql_value(const string &value) const
{
    if (mysql == NULL)
    {
        return "";
...
bool http_conn::has_user_session() const
{
    return !m_current_user.empty() && m_current_user != "admin";
}

http_conn::HTTP_CODE http_conn::respond_json_error(int status, const char *title, const string &message)
{
    char body[256];
...
http_conn::HTTP_CODE http_conn::require_user_session(const char *message)
{
    if (has_user_session())
    {
        return NO_REQUEST;
